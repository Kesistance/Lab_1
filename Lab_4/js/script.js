	...
	document.querySelectorAll('.box').forEach(image => {
		if (image.innerText.length !== 0) {
			if (image.innerText == 'Wrawn' || image.innerText == 'Bpawn') {
				image.innerHTML =
					'${image.innerText} <img class = "allimg allpaw" src = "${image.innerText}.png" alt="">';
				image.computedStyleMap.cursor = 'pointer';
			} else {
				image.innerHTML =
					'${image.innerText} <img class="allimg" src="${image.innerText}.png" alt="">';
			...
			if (item.innerText == '${toggle}bishop') {
				for (let i = 1; i < 9; i++) {
					if (
						i < (900 - aup) / 100 &&
						i < 9 - aside &&
						document.getElementById('b${a + i * 100 + i}').innerText == 0
					) {
